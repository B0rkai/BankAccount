...
: wxDialog(parent, wxID_ANY, title, parent->GetPosition() + wxPoint(50, 60), wxSize(XSIZE, YSIZE)) {
	// The size given to the constructor is the whole window including the title bar, so laying
	// out children against it directly (as below) leaves the client area shorter than YSIZE and
	// clips the bottom row. SetClientSize grows the window so the client area is exactly
