...
- **Where it's configured**: `db\location.json`, a small local (per-machine, hand-edited) JSON
  ...
  (deliberately, for v1). `release.json` (`ReleaseManifest`) is the same JSON-object shape, one
  level up the stack — machine-generated by `PackageRelease.ps1`, never hand-edited.
