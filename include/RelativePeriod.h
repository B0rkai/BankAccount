...
// "this_quarter", "last_quarter", "this_half", "last_half", "this_year", "last_year", plus
// rolling-window/multi-year ones the Periods menu itself has no shortcut for: "last_30_days"
// (today and the 29 days before it), "last_12_months" (the current calendar month and the 11
// before it), and "last_N_whole_years" for any positive integer N (the N full calendar years
// before the current one).
DateRange ResolveRelativePeriod(const String& keyword, const wxDateTime& today);

// Resolves a single-date relative keyword against `today` into `excel_date`, returning false (and
// leaving `excel_date` untouched) for an unrecognized keyword - the FavoriteQueryDef::date_from/
// date_to counterpart to ResolveRelativePeriod() above. Recognized keywords: "today",
