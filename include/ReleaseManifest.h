...
struct ReleaseManifest {
	...
	// A JSON object with recognized keys "version" (verbatim string) and "crc32" (a hex
	// string, e.g. "1A2B3C4D", case-insensitive - not a JSON number, to match the CRC's usual
	// hex display). Malformed JSON, a non-object root, or either key missing/non-string
	// leaves the corresponding field unset - valid ends up false, same as a missing file.
